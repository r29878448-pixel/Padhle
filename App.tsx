...
const App: React.FC = () => {
  ...
  if (isLoading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white gap-6">
      ...
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Delta Engine Initializing...</p>
    </div>
  );
      ...
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-md w-full space-y-12 animate-delta">
          ...
          <div className="space-y-4">
            ...
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px]">India's Premium Educational Platform</p>
          </div>
          <button onClick={() => setIsAuthModalOpen(true)} className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-3">
             <UserIcon size={20}/> Unlock My Classroom
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row font-sans text-slate-900">
      ...
      <aside className={`fixed md:sticky top-0 h-screen z-[150] w-72 bg-white border-r border-slate-100 flex flex-col transition-transform duration-500 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-10 border-b border-slate-50 flex items-center gap-4 cursor-pointer" onClick={() => setActiveView('home')}>
          ...
          <div>
            ...
            <span className="text-[7px] font-black text-blue-600 uppercase tracking-widest block mt-1.5">Premium Portal</span>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 relative pb-24 md:pb-0 overflow-y-auto">
        <header className="h-20 delta-glass flex items-center px-6 md:px-12">
           <div className="flex-1 flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-full border border-blue-100">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest">Live: {lastSync.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
              </div>
           </div>
           ...
           {activeView === 'home' && (
              <div className="space-y-16 animate-delta">
                 ...
                 <div className="relative rounded-[3.5rem] overflow-hidden shadow-2xl bg-slate-950 h-72 md:h-[34rem] group">
                    ...
                    <div className="absolute inset-0 flex flex-col justify-center px-10 md:px-24 bg-gradient-to-r from-slate-950 via-slate-950/20 to-transparent">
                       <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-600 text-white text-[9px] font-black uppercase tracking-[0.3em] rounded-full mb-8 w-fit">
                          <Zap size={14} className="fill-white"/> Real-Time Syllabus Sync
                       </div>
                       <h1 className="text-4xl md:text-8xl font-black text-white leading-[1.1] uppercase italic tracking-tighter">Your Path to<br/><span className="text-blue-500 underline decoration-white/10 underline-offset-[16px]">Academic Success.</span></h1>
                    </div>
                 </div>
                 ...
                 <div className="space-y-8">
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-3">
                       <Layers size={20} className="text-blue-600"/> Class Wise Segments
                    </h3>
                 ...
                 <section className="space-y-12">
                    <div className="flex items-center justify-between">
                       <h2 className="text-4xl font-black text-slate-900 uppercase italic tracking-tighter flex items-center gap-4">Current Batches</h2>
                       <div className="flex items-center gap-3">
                          <span className="w-3 h-3 bg-emerald-500 rounded-full animate-pulse"></span>
                          <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Enrollment Live</span>
                       </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                          ...
                          >
                             ...
                             <div className="p-12 flex-1 flex flex-col">
                                ...
                                <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-12 flex items-center gap-2">
                                   <UserIcon size={16} className="text-blue-500" /> {course.instructor || 'Delta Faculty Team'}
                                </p>
                                
                                <div className="mt-auto pt-10 border-t border-slate-50 flex items-center justify-between">
                                   <div className="flex flex-col">
                                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Academic Price</span>
                                      <div className="flex items-center gap-3">
                                   ...
                                   <div className="w-16 h-16 bg-slate-50 rounded-[2rem] flex items-center justify-center text-slate-300 group-hover:bg-blue-600 group-hover:text-white transition-all">
                       ...
                       {filteredCourses.length === 0 && (
                          <div className="col-span-full py-40 text-center bg-white border border-slate-100 rounded-[4rem] animate-pulse">
                             ...
                             <p className="text-slate-400 font-black uppercase tracking-[0.4em] text-[12px]">No batches synced in this segment yet.</p>
                          </div>
                       )}
                    </div>
                 </section>
                       ...
                       {selectedCourse.subjects.map(subject => (
                          <div key={subject.id} className="space-y-12">
                             <div className="flex items-center gap-6">
                               ...
                               <h3 className="text-4xl font-black text-slate-900 uppercase italic tracking-tighter">{subject.title} Core Units</h3>
                             </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 mb-12">
                        <div>
                          ...
                          <div className="flex items-center gap-6 mt-6">
                            ...
                            <span className="px-6 py-2.5 bg-slate-50 text-slate-400 rounded-2xl text-[11px] font-black uppercase tracking-widest">{selectedLecture.duration} Session</span>
                          </div>
                        </div>
                      </div>
                      
                      <p className="text-slate-600 text-lg font-medium leading-relaxed mb-16">{selectedLecture.description || "Expert-led class focusing on in-depth concept clarity and exam-level problem solving."}</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-slate-50 pt-16">
                         <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                               ...
                               {!selectedLecture.resources?.some(r => r.type === 'pdf') && <div className="p-8 border-2 border-dashed border-slate-100 rounded-[2rem] text-center text-[10px] font-black text-slate-300 uppercase">Awaiting class notes upload</div>}
                            </div>
                         </div>
                         ...
                         <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                               ...
                               {!selectedLecture.resources?.some(r => r.type === 'dpp') && <div className="p-8 border-2 border-dashed border-slate-100 rounded-[2rem] text-center text-[10px] font-black text-slate-300 uppercase">DPP sheets arriving soon</div>}
                            </div>
                         </div>
                      </div>
                    </div>
                  ...
                  <div className="lg:col-span-4 space-y-12">
                    <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm sticky top-32 overflow-hidden">
                       <h3 className="text-[13px] font-black text-slate-900 uppercase tracking-[0.3em] mb-12 flex items-center gap-3">
                          <Zap size={20} className="text-blue-600"/> Session Checklist
                       </h3>
                       <div className="space-y-6">
                          {[
                            { label: "Lecture Consumed", done: true },
                            { label: "Material Downloaded", done: false },
                            { label: "Unit DPP Completed", done: false }
                          ].map((item, idx) => (
                       ...
                       <div className="mt-12 p-8 bg-blue-600 rounded-[2.5rem] text-white shadow-2xl shadow-blue-500/30 text-center">
                          ...
                          <button className="w-full bg-white text-blue-600 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest hover:bg-slate-50 transition-all">Join Peer Group</button>
                       </div>
                    </div>
                  </div>
                </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-24 bg-white/95 backdrop-blur-2xl border-t border-slate-100 flex items-center px-4 z-[200] pb-2 shadow-[0_-20px_60px_rgba(0,0,0,0.06)]">
           ...
           <BottomNavItem icon={<UserIcon size={24}/>} label="Me" active={activeView === 'profile'} onClick={() => setActiveView('profile')}/>
        </nav>
      </div>
